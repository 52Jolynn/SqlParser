...
WHITE_SPACE : ' ' -> skip;//skip when match this token
TAB : '\t' -> skip; //tab
NEWLINE : '\r'? '\n' -> skip;
RESERVED_WHITE_SPACE : ' '; //don't skip when match this token, reserved.
...
fragment
    DAY_TIME_INTERVAL : DAYS_VALUE (RESERVED_WHITE_SPACE HOURS_VALUE (COLON MINUTES_VALUE (COLON SECONDS_VALUE)?)?)?;//31 12:59:59
fragment
...
TIMESTAMP_STRING : QUOTE DATE_VALUE RESERVED_WHITE_SPACE TIME_VALUE TIME_ZONE_INTERVAL? QUOTE;
...
COMMENT : COMMENT_INTRODUCER (NONQUOTER_STRING|QUOTER_SYMBOL)* NEWLINE -> skip;//comment
BIT_STRING_LITERAL : B QUOTE BINARY* QUOTE; //'100001111'
HEX_STRING_LITERAL : X QUOTE (HEX HEX HEX HEX)? QUOTE;//'FFEE'
NATIONAL_CHARACTER_STRING_LITERAL : N QUOTE_STRING_LITERAL;
QUOTE_STRING_LITERAL : QUOTE NONQUOTER_STRING+ QUOTE|QUOTER_SYMBOL;
DOUBLEQUOTE_STRING_LITERAL : DOUBLE_QUOTE NONDOUBLEQUOTER_STRING+ DOUBLE_QUOTE|DOUBLEQUOTER_SYMBOL;
IDENTIFIER : ALPHA (UNDERSCORE|DIGIT|ALPHA)*;
